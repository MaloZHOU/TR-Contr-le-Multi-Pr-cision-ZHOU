...
function Generate_Trap_nhs_tol_ep(nhs,tol,ep)
    p = Plots.plot(title = "tol = $tol, epsilon = $ep, Trapezoidal",xlabel="Temps", ylabel="Value")
    for nh in nhs
...
for tol in tols
    for ep in eps
        p = Generate_Trap_nhs_tol_ep(nhs,tol,ep)
        Plots.savefig("Photo/Comparaison_nhs_tol_ep/Trapezoidal,tol = $tol, epsilon = $ep.png")
