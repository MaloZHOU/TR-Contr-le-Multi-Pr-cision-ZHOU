...
function Generate_thrust_hersim(nhs=nhs)
    P = Plots.plot(title = "tol = 1e-8, Hermite_Simpson",xlabel="Time", ylabel="Thrust Values")
...
function Generate_thrust_hersim_tol(tols,nh = 500)
    P = Plots.plot(title = "nh = $nh, Hermite_Simpson",xlabel="Time", ylabel="Thrust Values")
...
function rocket_model_hersim_res(nh,coef_obj)
    h_0 = 1.0   #hauteur init
    v_0 = 0.0   #vitesse init
    m_0 = 1.0   #masse init
    g_0 = 1.0   #grativité init
    ...
    @objective(model, Min, (-coef_obj)*h[nh,0] +sum( wh * (h[i+1,0] - h[i,0]-step*dh[i,0])^2 
                                                    ...
                                                    + wm * (m[i+1,0] - m[i,0]-step*dm[i,0])^2 for i in 0:nh-1))
...
function Generate_thrust_hersim_obj(nh,coef_objs=objs)
    P = Plots.plot(xlabel="Time", ylabel="Thrust Value",title = "nh=$nh, Hermite-Simpson")
    for i in range(1,length(coef_objs))
        println(coef_objs[i])
        coef_obj = coef_objs[i]
        model = rocket_model_hersim_res(nh,coef_obj)
        JuMP.set_optimizer(model, Ipopt.Optimizer)
        JuMP.set_attribute(model, "hsllib", HSL_jll.libhsl_path)
        JuMP.set_attribute(model, "linear_solver", "ma57")
        JuMP.optimize!(model)
...
objs = [0,0.1,0.5,0.9,1.0]
# Generate_thrust_hersim_obj(300,[0,0.1,0.5,0.9,1])
